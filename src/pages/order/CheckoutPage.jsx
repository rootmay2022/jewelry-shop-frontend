...
const CheckoutPage = () => {
    ...
    const location = useLocation(); // Thêm cái này để lấy data "Mua Ngay"
    ...
    // 1. CHUẨN HÓA DATA: Gom "Mua Ngay" và "Giỏ Hàng" về một mối
    const checkoutData = useMemo(() => {
        // Trường hợp 1: Mua Ngay từ trang sản phẩm
    ...
    const onFinish = async (values) => {
        ...
        try {
            const orderPayload = {
                ...values,
                userId: user?.id,
                totalAmount: checkoutData.totalAmount, // Lấy chuẩn từ checkoutData
                isVAT: isCompany,
    ...
    // 2. FIX LỖI TREO SPIN: Nếu có checkoutData thì cho qua luôn
    if (!checkoutData) {
        return (
    ...
    return (
        <div style={{ backgroundColor: '#f4f4f4', minHeight: '100vh', padding: '40px 0' }}>
                        ...
                        <Col xs={24} lg={10}>
                            <Card bordered={false} className="receipt-card">
                                ...
                                <div className="receipt-items">
                                    {/* Dùng checkoutData thay vì cart */}
            ...
            {/* --- 5. TEMPLATE HÓA ĐƠN PDF (Cũng dùng checkoutData) --- */}
